...
func (war *Warrior) init() {
	...
	if !TeamsEnabled {
		war.unit.SetDisplayName("Lance", nil)
	} else {
...
func (war *Warrior) useAbilities() {
	if war.abilities.isAlive {
		if (ns.InCirclef{Center: war.unit, R: 150}.Matches(war.target)) && !war.target.Flags().Has(object.FlagDead) {
			war.useHarpoon()
...
func (war *Warrior) onEnemyHeard() {
	if ns.GetCaller() != nil {
		if ns.GetCaller().HasEnchant(enchant.INVISIBLE) {
			war.useEyeOfTheWolf()
		}
	}
	war.ThrowChakram()
...
func (war *Warrior) onHit() {
	war.mostRecentAttacker = ns.GetCaller()
	if war.abilities.HarpoonAttached {
		war.onHarpoonStop()
	}
	if war.unit.CurrentHealth() < 100 && !war.behaviour.Busy && war.target.CurrentHealth() > 10 {
		war.GoToRedPotion()
	}
}
...
func (war *Warrior) useBerserkerCharge() {
	//rnd := 0
	//if (ns.InCirclef{Center: war.unit, R: 150}.Matches(war.target)) {
	//	rnd = ns.Random(1, 3)
	//	}
	//if (ns.InCirclef{Center: war.unit, R: 100}.Matches(war.target)) {
	//	rnd = ns.Random(1, 2)
	//	}
	//if (ns.InCirclef{Center: war.unit, R: 50}.Matches(war.target)) {
	//	rnd = 1
	//}
	//if rnd != 1 {
	//	return
...
func (war *Warrior) BerserkerChargeCooldownManager() {
	if !war.unit.Flags().Has(object.FlagDead) {
...
func (war *Warrior) useWarCry() {
	...
	if war.abilities.WarCryReady && !war.abilities.BerserkerChargeActive && !war.abilities.HarpoonFlying && war.abilities.Ready && war.unit.CanSee(war.target) && !war.target.Flags().Has(object.FlagDead) {
...
func (war *Warrior) useHarpoon() {
	if war.abilities.HarpoonReady && !war.target.HasEnchant(enchant.INVULNERABLE) && !war.abilities.BerserkerChargeActive && war.abilities.Ready && war.unit.CanSee(war.target) {
		...
		war.abilities.HarpoonMask.FlagsDisable(object.FlagEnabled)
...
func (war *Warrior) onHarpoonHit() {
	...
	if !war.abilities.HarpoonTarget.HasEnchant(enchant.SHOCK) {
...
func (war *Warrior) onHarpoonReelLoop() {
	if war.unit.CanSee(war.abilities.HarpoonTarget) && war.abilities.HarpoonAttached && (ns.InCirclef{Center: war.unit, R: 300}.Matches(war.abilities.HarpoonTarget)) && !war.abilities.HarpoonTarget.Flags().Has(object.FlagDead) && war.abilities.isAlive {
		...
		if war.abilities.HarpoonTarget != ns.FindClosestObject(war.abilities.HarpoonMask, ns.HasTypeName{"CarnivorousPlant"}) {
...
func (war *Warrior) onHarpoonStop() {
	ns.AudioEvent(audio.HarpoonBroken, war.unit)
	war.abilities.HarpoonFlying = false
	war.abilities.HarpoonAttached = false
	war.abilities.HarpoonMask.Delete()
